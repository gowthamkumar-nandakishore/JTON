...
# Invalid RFC 8259 files we intentionally accept because the MYSON parser ships
...
def test_json_parsing_corpus(case_path: Path, prefix: str) -> None:
    ...
    if prefix == "y":
        ...
        result = myson.loads(text)
        assert result == reference
    else:
        if case_path.name in PARSING_EXTENSION_ACCEPTED:
            assert myson.loads(text) is not None
        else:
...
def test_json_checker_pass_cases(case_path: Path) -> None:
    text = _read_json_text(case_path)
    assert myson.loads(text) == json.loads(text)
