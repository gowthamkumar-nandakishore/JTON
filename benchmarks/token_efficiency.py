...
def format_dataset_result(result: BenchmarkResult) -> str:
    ...
    JTON = next(f for f in result.formats if f.name == "JTON")
    ...
    if csv_format and result.tabular_percent == 100:
        ...
        bar = create_bar_chart(jton.tokens, max_tokens)
        JTON_str = f"{jton.tokens:,}".rjust(TOKEN_PADDING)
        csv_diff = jton.tokens - csv_format.tokens
