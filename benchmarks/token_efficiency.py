...
# Format order for comparisons (against MYSON as baseline)
...
class FormatMetrics:
    ...
    name: str
    tokens: int
    savings: int  # Negative = MYSON uses fewer tokens
...
def benchmark_dataset(dataset_name: str) -> BenchmarkResult:
    ...
    # Calculate savings relative to MYSON
    results = []
    for metrics in format_metrics:
        savings = metrics["tokens"] - myson_tokens
...
def format_comparison_line(format_metrics: FormatMetrics, is_last: bool = False) -> str:
    """Format a comparison line showing savings vs MYSON"""
    label = FORMATTER_DISPLAY_NAMES.get(format_metrics.name, format_metrics.name.upper())
    
    # Sign for savings (negative means MYSON saves tokens)
...
def format_dataset_result(result: BenchmarkResult) -> str:
    ...
    # Get MYSON and other format metrics
    myson = next(f for f in result.formats if f.name == "myson")
    ...
    # MYSON baseline (first format might be CSV if available)
    ...
    if csv_format and result.tabular_percent == 100:
        ...
        # Show MYSON with comparison to CSV
        bar = create_bar_chart(myson.tokens, max_tokens)
        myson_str = f"{myson.tokens:,}".rjust(TOKEN_PADDING)
        csv_diff = myson.tokens - csv_format.tokens
        csv_diff_pct = (csv_diff / csv_format.tokens * 100) if csv_format.tokens > 0 else 0
        sign = "+" if csv_diff >= 0 else "−"
        lines.append(f"   MYSON               {bar}    {myson_str} tokens   ({sign}{abs(csv_diff_pct):.1f}% vs CSV)")
    else:
        # Show MYSON as baseline
        bar = create_bar_chart(myson.tokens, max_tokens)
        myson_str = f"{myson.tokens:,}".rjust(TOKEN_PADDING)
        lines.append(f"   MYSON               {bar}    {myson_str} tokens")
...
def format_track_totals(totals: Dict[str, int], track_name: str) -> str:
    ...
    if csv_total > 0:
        ...
        # Show MYSON with CSV comparison
        bar = create_bar_chart(myson_total, max_tokens)
        myson_str = f"{myson_total:,}".rjust(TOKEN_PADDING)
        csv_diff = myson_total - csv_total
