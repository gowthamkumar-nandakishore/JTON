...
# Format order for comparisons (against JTON as baseline)
...
class FormatMetrics:
    ...
    name: str
    tokens: int
    savings: int  # Negative = JTON uses fewer tokens
...
def benchmark_dataset(dataset_name: str) -> BenchmarkResult:
    ...
    # Calculate savings relative to JTON
    results = []
    for metrics in format_metrics:
        savings = metrics["tokens"] - JTON_tokens
...
def format_comparison_line(format_metrics: FormatMetrics, is_last: bool = False) -> str:
    """Format a comparison line showing savings vs JTON"""
    label = FORMATTER_DISPLAY_NAMES.get(format_metrics.name, format_metrics.name.upper())
    
    # Sign for savings (negative means JTON saves tokens)
...
def format_dataset_result(result: BenchmarkResult) -> str:
    ...
    # Get JTON and other format metrics
    jton_fmt = next(f for f in result.formats if f.name == "JTON")
    ...
    # JTON baseline (first format might be CSV if available)
    ...
    if csv_format and result.tabular_percent == 100:
        ...
        # Show JTON with comparison to CSV
        bar = create_bar_chart(jton_fmt.tokens, max_tokens)
        JTON_str = f"{jton_fmt.tokens:,}".rjust(TOKEN_PADDING)
        csv_diff = jton_fmt.tokens - csv_format.tokens
        csv_diff_pct = (csv_diff / csv_format.tokens * 100) if csv_format.tokens > 0 else 0
        sign = "+" if csv_diff >= 0 else "−"
        lines.append(f"   JTON               {bar}    {JTON_str} tokens   ({sign}{abs(csv_diff_pct):.1f}% vs CSV)")
    else:
        # Show JTON as baseline
        bar = create_bar_chart(jton_fmt.tokens, max_tokens)
        JTON_str = f"{jton_fmt.tokens:,}".rjust(TOKEN_PADDING)
        lines.append(f"   JTON               {bar}    {JTON_str} tokens")
...
def format_track_totals(totals: Dict[str, int], track_name: str) -> str:
    ...
    if csv_total > 0:
        ...
        # Show JTON with CSV comparison
        bar = create_bar_chart(JTON_total, max_tokens)
        JTON_str = f"{JTON_total:,}".rjust(TOKEN_PADDING)
        csv_diff = JTON_total - csv_total
